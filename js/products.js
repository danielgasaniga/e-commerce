...
function setProdID(id) {
  //Realizo una funcion para traer el local storage, y me rediriga a product-info.html
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.getElementById("asc").addEventListener("click", function () {
    //Hago que ralice la funcion de ascender al apretar el boton click
    funcAscendente();
  });

  document.getElementById("desc").addEventListener("click", function () {
    //Hago que ralice la funcion de descender al apretar el boton click
    funcDescendente();
  });

  document.getElementById("rel").addEventListener("click", function () {
    //Hago que ralice la funcion de relevancia al apretar el boton click
    funcRelevancia();
  });
});

function funcAscendente() {
  //Creo la funcion ascendente, haciendo el orden para que me muestre el producto de mayor precio, al de menor precio
...
function funcDescendente() {
  //Creo la funcion descendente, haciendo el orden para que me muestre el producto de menor precio, al de mayor precio
  listaProductos.sort((a, b) => {
    if (a.cost < b.cost) {
      return -1;
    }
...
function funcRelevancia() {
  //Creo la funcion relevancia, haciendo el orden para que me muestre desde el producto mas venido, al de menos venta
  listaProductos.sort((a, b) => {
