  ...
  .then((infoComments) => {
    for (let commentsData of infoComments) {
      info_comments.innerHTML += `<li class="list-group-item comments-list">
        <p class = "comments-list-head"><b>${commentsData.user}</b> - ${
        commentsData.dateTime
      }  
        <span class="fa fa-star ${
          commentsData.score >= 1 ? "checked" : ""
        }"></span>
        <span class="fa fa-star ${
          commentsData.score >= 2 ? "checked" : ""
        }"></span>
        <span class="fa fa-star ${
          commentsData.score >= 3 ? "checked" : ""
        }"></span>
        <span class="fa fa-star ${
          commentsData.score >= 4 ? "checked" : ""
        }"></span>
        <span class="fa fa-star ${
          commentsData.score >= 5 ? "checked" : ""
        }"></span>
...
botonEnviar.addEventListener("click", (evt) => {
  ...
  if (nuevoComentario.value)
    localStorage.setItem("comentarios", nuevoComentario.value);
});
...
info_comments.innerHTML = `<li class="list-group-item comments-list">
  ...
  <span class="fa fa-star ${
    conectarComentarios.score >= 1 ? "checked" : ""
  }"></span>
  <span class="fa fa-star ${
    conectarComentarios.score >= 2 ? "checked" : ""
  }"></span>
  <span class="fa fa-star ${
    conectarComentarios.score >= 3 ? "checked" : ""
  }"></span>
  <span class="fa fa-star ${
    conectarComentarios.score >= 4 ? "checked" : ""
  }"></span>
  <span class="fa fa-star ${
    conectarComentarios.score >= 5 ? "checked" : ""
  }"></span>
  <p class = "comments-list-data">${conectarComentarios}</p>
  </li> `;

function cargarRelacionados(id) {
  //Aqui realizo una funcion para enviar al local storage el producto, y me redirija
  localStorage.setItem("prodID", id);
  window.location = "product-info.html";
}

document.addEventListener("DOMContentLoaded", function () {
  //Realizo un addeventlistener para traer la informacion del json
...
function relacionando() {
  //Creo esta funcion, para traer la informacian de los productos relacionados, trayendo su imagen, y nombre, y a la vez realizo un evento onclick, para que al cargar dichos productos, al hacer click, me redirija hacia la informacion de los mismos
